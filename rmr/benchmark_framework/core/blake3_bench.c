 ...
 *
 * Common benchmark harness. It is linked unchanged against the selected
 * BLAKE3 library; the loop is not part of the cryptographic implementation.
 */
...
int main(int argc, char **argv) {
    ...
    unsigned long iterations = 64;

    if(argc > 2) {
        iterations = strtoul(argv[2], NULL, 10);
        if(iterations == 0) {
            fputs("iterations must be greater than zero\n", stderr);
            return 2;
        }
    }
    ...
    struct timespec end;
    if(clock_gettime(CLOCK_MONOTONIC, &begin) != 0) return 3;
    ...
    printf("RESULT,%s,%.9f,%.3f,%08x,", mode, seconds, mib_per_second, guard);
