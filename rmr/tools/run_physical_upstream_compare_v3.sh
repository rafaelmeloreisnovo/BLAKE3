...
#
# Device-bound upstream-vs-fork comparison.
# COMPILE != PHYSICAL_EXECUTION; this script becomes physical evidence only
# when executed to completion on the target device.
set -euo pipefail
...
for t in git cmake "$CC_BIN" "$CXX_BIN" python3 sha256sum; do
  command -v "$t" >/dev/null 2>&1 || { echo "missing_command=$t" >&2; exit 127; }
done
...
case "$ARCH" in
  armv7l|armv8l|arm)
    SIMD_ARGS=("-DBLAKE3_SIMD_TYPE=neon-intrinsics" "-DBLAKE3_CFLAGS_NEON=-mfpu=neon-vfpv4")
...
OFFICIAL_LIB="$(find "$OFFICIAL_BUILD" -name libblake3.a -type f -print -quit)"
FORK_LIB="$(find "$FORK_BUILD" -name libblake3.a -type f -print -quit)"
...
for size in $SIZES; do
  ...
  for ((r=1;r<=ROUNDS;r++)); do
    if ((r%2)); then
      append_result "$OFFICIAL_BUILD/bench" official "$size" "$iters" "$r"
      append_result "$FORK_BUILD/bench" fork "$size" "$iters" "$r"
    else
      append_result "$FORK_BUILD/bench" fork "$size" "$iters" "$r"
      append_result "$OFFICIAL_BUILD/bench" official "$size" "$iters" "$r"
    fi
...
python3 "$ROOT/rmr/benchmark_framework/simperf/analyze_blake3_compare_v2.py" \
  --csv "$OUT/results.csv" --out "$OUT/summary.json" \
